...
from .Commands.Types import (BaseCommand, BrowseCommand, DumpPageSourceCommand,
                             ...
                             DumpCookieNoticeCommand, SaveCookieNoticeScreenshotCommand)
...
class CommandSequence:
    ...
    def dump_cookie_notice(self, suffix='', timeout=60):
        """ Searches for the website's cookie notice in the source code
            and if found is dumped in a file
        """
        self.total_timeout += timeout
        if not self.contains_get_or_browse:
            raise CommandExecutionError("No get or browse request preceding "
                                        "the dump cookie notice command", self)
        command = DumpCookieNoticeCommand(suffix)
        self._commands_with_timeout.append((command, timeout))

    def screenshot_cookie_notice(self, suffix='', timeout=30):
        """ Takes a screenshot of the website's cookie notice"""
        self.total_timeout += timeout
        if not self.contains_get_or_browse:
            raise CommandExecutionError("No get or browse request preceding "
                                        "the save screenshot command", self)
        command = SaveCookieNoticeScreenshotCommand(suffix)
        self._commands_with_timeout.append((command, timeout))
