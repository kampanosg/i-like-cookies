...
from .utils.webdriver_utils import (execute_in_all_frames,
                                    ...
                                    wait_until_loaded)
from .utils.cookie_utils import find_cookie_banner_website, save_cookie_banner_to_db
from ..Classes.CookieBanner import CookieBanner
...
def _create_and_return_output_directory(path='', url=''):
    # Create the {path}/cookies/ directory if it doesn't already exist
    cookies_dir = os.path.join(path, 'cookies', '')
    if not os.path.exists(cookies_dir):
        os.mkdir(cookies_dir)

    # Create the directory to store the cookie HTML
    url_hash = md5(url.encode('utf-8')).hexdigest()
    output_directory = os.path.join(cookies_dir, '{}'.format(url_hash), '')

    if not os.path.exists(output_directory):
        os.mkdir(output_directory)
        logger.info('DumpCookieNoticeCommand: Created directory with name {}'.format(output_directory))

    return output_directory, url_hash


def dump_cookie_notice(visit_id, driver, manager_params, suffix=''):
    output_path, url_hash = _create_and_return_output_directory(path=manager_params['cookie_directory'],
                                                      url=driver.current_url)
    outfile = '{}cookie-notice.html'.format(output_path)

    page_source = driver.page_source.encode('utf8')
    cookie_css_selectors = manager_params['cookie_css_selectors']
    found_cookie_selector = find_cookie_banner_website(css_selectors=cookie_css_selectors, page_source=page_source)

    cookie_banner = CookieBanner(banner_hash=url_hash, website=driver.current_url, visit_id=visit_id)

    if found_cookie_selector is not None:
        logger.info('DumpCookieNoticeCommand: Found cookie notice in elements {}'.format(found_cookie_selector))

        # Find the HTML element(s) using Selenium
        elements = driver.find_elements_by_css_selector(found_cookie_selector)
        if len(elements) > 0:
            ...
            # Use the first element for storing x,y, width, height, etc. values (not very important)
            element = elements[0]
            cookie_banner.width = element.size['width']
            cookie_banner.height = element.size['height']
            cookie_banner.position_x = element.location['x']
            cookie_banner.position_y = element.location['y']
            cookie_banner.selector = found_cookie_selector

            # If no HTML was found or the html found didn't have 'cookie' in it then manual inspection is needed
            if len(cookie_banner_html.strip()) > 0 and 'cookie' in cookie_banner_html:
                logger.info('DumpCookieNoticeCommand: Successfully found cookie notice in element: {}'
                            .format(found_cookie_selector))
                cookie_banner.html = cookie_banner_html
                cookie_banner.banner_exists = 1
            else:
                logger.info('DumpCookieNoticeCommand: Element did not contain cookie notice - Marked as false positive')
    else:
        logger.info('DumpCookieNoticeCommand: Cookie notice was not found. Manual intervention is required'.format(
            found_cookie_selector))

    logger.info('DumpCookieNoticeCommand: Saving cookie banner to db')
    save_cookie_banner_to_db(manager_params['cookie_db'], cookie_banner)

    try:
        with open(outfile, 'wb') as f:
            f.write(page_source)
            f.write(b'\n')
        logger.info('DumpCookieNoticeCommand: Successfully wrote source code to file {}'.format(outfile))
    except BaseException as base_exception:
        logger.error(
            'DumpCookieNoticeCommand: Unable to write source code to file {}. {}'.format(outfile, base_exception))


def screenshot_cookie_notice(visit_id, driver, manager_params, suffix=''):
    output_path, _ = _create_and_return_output_directory(path=manager_params['cookie_directory'],
                                                      url=driver.current_url)
    outfile = '{}screenshot.png'.format(output_path)

    try:
        driver.save_screenshot(outfile)
        logger.info('SaveScreenshotCookieNoticeCommand: Successfully saved screenshot to file {}'.format(outfile))
    except BaseException as base_exception:
        logger.error(
            'SaveScreenshotCookieNoticeCommand: Unable to save screenshot to file {}. {}'.format(outfile,
                                                                                                 base_exception))
