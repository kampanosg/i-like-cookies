...
def execute_command(command, webdriver, browser_settings, browser_params,
    ...
    elif type(command) is DumpCookieNoticeCommand:
        browser_commands.dump_cookie_notice(
            visit_id=command.visit_id, driver=webdriver,
            manager_params=manager_params, suffix=command.suffix)

    elif type(command) is SaveCookieNoticeScreenshotCommand:
        browser_commands.screenshot_cookie_notice(
            visit_id=command.visit_id, driver=webdriver,
            manager_params=manager_params, suffix=command.suffix)
